...
async function main() {
  ...
  const contractPath = path.join(root, "contracts", "src", "PrivatePredictionMarket.sol");
  const compileResult = compileContract(contractPath);
  const contract = compileResult.contracts["PrivatePredictionMarket.sol"]?.PrivatePredictionMarket;
  if (!contract?.abi || !contract?.evm?.bytecode?.object) {
    throw new Error("Compilation succeeded but PrivatePredictionMarket artifact missing");
  }
  ...
  fs.writeFileSync(
    path.join(deploymentDir, "private-prediction-market.json"),
    JSON.stringify(deployment, null, 2)
  );

  setEnvValue(path.join(root, ".env"), "PRIVATE_PREDICTION_MARKET_ADDRESS", address);
  ...
  setEnvValue(webEnvLocal, "VITE_PRIVATE_PREDICTION_MARKET_ADDRESS", address);
  ...
  setEnvValue(frontendEnvLocal, "VITE_PRIVATE_PREDICTION_MARKET_ADDRESS", address);
  setEnvValue(frontendEnvLocal, "VITE_CHAIN_ID", String(chainId));
  setEnvValue(frontendEnvLocal, "VITE_RPC_URL", rpcUrl);
  setEnvValue(frontendEnvLocal, "VITE_API_URL", process.env.VITE_API_URL || "http://localhost:8787");
  setEnvValue(
    frontendEnvLocal,
    "VITE_USDC_ADDRESS",
  ...
  setEnvValue(apiEnvLocal, "PRIVATE_PREDICTION_MARKET_ADDRESS", address);
  ...
  console.log("[deploy] wrote artifacts/deployment/private-prediction-market.json and synced env files");
}
